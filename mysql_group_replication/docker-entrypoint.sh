...
_main() {
	...
	if [ "$1" = 'mysqld' ] && ! _mysql_want_help "$@"; then
		mysql_note "Modified entrypoint script for MySQL Server ${MYSQL_VERSION} started."
		...
		if [ -z "$DATABASE_ALREADY_EXISTS" ]; then
		    mysql_note "empty DATABASE_ALREADY_EXISTS"
			docker_verify_minimum_env
		    ...
		    mysql_note "else DATABASE_ALREADY_EXISTS"
		fi
	fi
	# set +x
	exec "$@"
